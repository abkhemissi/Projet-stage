...
class ContEtudiant extends ContGenerique{
    ...
    function form_addEtud(){
        ...
        $numApo = $numINE = $nomEtud = $prenom = $dateNaiss = $courriel = $tel = $adrr1 = $adrr2 = $AnneePromo = $situationActu = $etablissements = "";
    ...
    function add_Student(){
        ...
        $idEtud = $this->modeleEtud->add_studentBD(htmlspecialchars($_POST['numApogee']),htmlspecialchars($_POST['numeroINE']),$photoEtud,htmlspecialchars($_POST['nom']),htmlspecialchars($_POST['prenom']),htmlspecialchars($_POST['dateNaiss']),
    ...
    function affichage_tri($tri,$recherche){

        if (strcmp($tri, "numINE") == 0){
            $nbEtudiants = $this->modeleEtud->nb_trie_INE($recherche);
            $nbPagesTotales = ceil($nbEtudiants/5);
            if (isset($_GET['page']) and !empty($_GET['page'])) {
                $page = $_GET['page'];
                $listEtudiantTri = $this->modeleEtud->trie_INE($page, $recherche);
        ...
        if (strcmp($tri, "numApogee") == 0){
            $nbEtudiants = $this->modeleEtud->nb_trie_apogee($recherche);
            $nbPagesTotales = ceil($nbEtudiants/5);
            if (isset($_GET['page']) and !empty($_GET['page'])) {
                $page = $_GET['page'];
                $listEtudiantTri = $this->modeleEtud->trie_apogee($page, $recherche);
                require('modules/mod_etudiant/vue_etudiant/listEtud.php');
            }
            else {
                $page = 1;
        ...
        if (strcmp($tri, "ecoleEntreprise") == 0){
    ...
    function update_Etud(){
        ...
        $photoEtud = self::fichier_image($_FILES);
        $this->modeleEtud->update_studentBD(htmlspecialchars($_POST['numApogee']),htmlspecialchars($_POST['numeroINE']),$photoEtud,htmlspecialchars($_POST['nom']),htmlspecialchars($_POST['prenom']),htmlspecialchars($_POST['dateNaiss']),
        htmlspecialchars($_POST['courriel']),htmlspecialchars($_POST['tel']),htmlspecialchars($_POST['adr1']),htmlspecialchars($_POST['adr2']),htmlspecialchars($_POST['anneePromotion']),htmlspecialchars($_POST['situationActuelle']),
    ...
    public function delete_EtabDeEtudiant(){
        $this->contEtab->supp_EtabDeEtudiantBD(htmlspecialchars($_GET['numeroINE']),htmlspecialchars($_GET['idEtab']));
        self::form_updateEtud();
        $message = 'SUPPRIMER';
        require('modules/mod_etudiant/vue_etudiant/notification.php');
        echo '<script src="modules/mod_etudiant/script/adresse.js" ></script>';
    ...
    function delete_student($id){
        $this->modeleEtud->delete_studentBD(htmlspecialchars($_GET['numeroINE']),$id);
